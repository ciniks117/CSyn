...
function [phi,rob,BrFalse] = init_robotarm(newfile,specno,mode)
    ...
    phi_c = STL_Formula('phi_c', 'ev_[0,tau2] alw ((abs(tAngle[t]-tREF[t]) < epsi2) and (abs(fAngle[t]-fREF[t]) < epsi2) and (abs(wAngle[t]-wREF[t]) < epsi2) and (abs(hAngle[t]-hREF[t]) < epsi2) and (abs(gAngle[t]-gREF[t]) < epsi2))');
    phi_c = set_params(phi_c,{'tau2', 'epsi2'}, [10 0.2]);
    ...
    x_gen = var_step_signal_gen({'hREF','gREF'});
